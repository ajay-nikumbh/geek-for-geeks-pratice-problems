...
|---|---|---|---|---|---|---|---|---|---|
| 1 | [Greedy](#greedy) | Sort or scan once, then make the locally optimal choice at each step (proven safe by an exchange argument) to reach the global optimum. | O(n log n) | O(1) | 35 | 0 | 20 | 15 | 0 |
| 2 | [Single Pass Linear Scan](#single-pass-linear-scan) | Walk the array once tracking a running value (max, min, sum, count) — no extra structure, just an accumulator updated as you go. | O(n) | O(1) | 34 | 11 | 23 | 0 | 0 |
| 3 | [Two Pointer](#two-pointer) | Two indices move from opposite ends or in tandem (often after sorting) to find pairs, partition, or rearrange in one linear pass. | O(n) or O(n log n) with sort | O(1) | 34 | 5 | 19 | 8 | 2 |
| 4 | [Hashing / Frequency Counting](#hashing-frequency-counting) | Tally occurrences via a hash map or frequency array to answer duplicate, count, or membership questions in one pass. | O(n) | O(n) | 24 | 5 | 9 | 10 | 0 |
| 5 | [Sorting-based](#sorting-based) | Sort the array (or a copy) first, then read the answer directly off the sorted order. | O(n log n) | O(1) to O(n) | 23 | 6 | 17 | 0 | 0 |
| 6 | [Prefix / Suffix Aggregate](#prefix-suffix-aggregate) | Precompute running sum/max/product from the left and/or right so any later query is answered in O(1) instead of rescanning. | O(n) | O(n) | 22 | 6 | 10 | 6 | 0 |
| 7 | [Binary Search on Sorted Array](#binary-search-on-sorted-array) | Binary search directly over a sorted (or rotated-sorted) array to locate a boundary, index, or element in logarithmic time. | O(log n) | O(1) | 20 | 0 | 12 | 8 | 0 |
| 8 | [Math / Number-Theoretic Construction](#math-number-theoretic-construction) | Apply arithmetic or number-theory reasoning — divisibility, digit construction, modulo, permutation counting — over the array values. | O(n) to O(n log n) | O(1) to O(n) | 16 | 0 | 7 | 9 | 0 |
| 9 | [In-Place Rearrangement](#in-place-rearrangement) | Restructure the array (rotate, swap, segregate) using only index arithmetic and O(1) extra space, no auxiliary array. | O(n) | O(1) | 12 | 6 | 0 | 6 | 0 |
| 10 | [Sliding Window](#sliding-window) | Maintain a contiguous window (fixed or variable size) across the array, expanding/shrinking it to track a running property. | O(n) | O(1) | 11 | 0 | 11 | 0 | 0 |
| 11 | [Array Modification / Insertion / Access](#array-modification-insertion-access) | Fundamental array operations — shifting to insert, accessing by index, transforming each element by a fixed rule. | O(n) | O(1) | 8 | 8 | 0 | 0 | 0 |
| 12 | [Digit / Bit Manipulation](#digit-bit-manipulation) | Operate on the digits or binary representation of the numbers themselves rather than array structure — XOR, parity, digit diffs. | O(n) or O(n log(max val)) | O(1) | 8 | 4 | 0 | 3 | 1 |
| 13 | [Binary Search on Answer](#binary-search-on-answer) | Binary search over the space of possible answers (a rate, capacity, distance), verifying each candidate with an O(n) feasibility check. | O(n log(range)) | O(1) | 8 | 0 | 0 | 4 | 4 |
| 14 | [K-th Element / Order Statistics](#k-th-element-order-statistics) | Find the k-th smallest/largest or median across one or two arrays via merge-style walk, partitioning, or a heap of size k. | O(log(min(n,m))) to O(n log k) | O(1) to O(k) | 8 | 0 | 0 | 6 | 2 |
| 15 | [XOR / Prefix-XOR Techniques](#xor-prefix-xor-techniques) | Exploit XOR's self-cancelling property, often paired with prefix-XOR and hashing, to count subarrays or isolate unique elements. | O(n) | O(n) | 8 | 0 | 0 | 8 | 0 |
| 16 | [Subset / Combinatorial Selection](#subset-combinatorial-selection) | Enumerate or DP over subsets — products, XORs, differences of chosen elements — where the search space is combinatorial. | O(2^n) brute / O(n·sum) DP | O(sum) or O(2^n) | 7 | 0 | 0 | 7 | 0 |
| 17 | [Duplicate / Distinct Element Handling](#duplicate-distinct-element-handling) | Identify, remove, or sum distinct/duplicate values, relying on sorting or hashing to detect repeats efficiently. | O(n) or O(n log n) | O(n) | 5 | 5 | 0 | 0 | 0 |
| 18 | [Stock Buy-Sell / Sequential Decision DP](#stock-buy-sell-sequential-decision-dp) | Single-pass or DP-style scan tracking running best/worst values to decide optimal buy/sell or transaction points. | O(n) | O(1) | 4 | 0 | 0 | 4 | 0 |
| 19 | [Monotonic Run / Bitonic Subarray](#monotonic-run-bitonic-subarray) | Identify the longest extremal contiguous run of a monotonic, alternating, or bitonic (increase-then-decrease) shape in one directional scan. | O(n) | O(1) | 4 | 0 | 0 | 4 | 0 |
| 20 | [Cyclic Sort / Index-Value Mapping](#cyclic-sort-index-value-mapping) | For values bounded in range 1..n, place each value at its 'correct' index in one pass to reveal missing/duplicate elements. | O(n) | O(1) | 3 | 0 | 3 | 0 | 0 |
| 21 | [Reconstruction / Simulation](#reconstruction-simulation) | Reconstruct an original array from derived data, or simulate a described process step by step to compute the final outcome. | O(n) to O(n log n) | O(n) | 3 | 0 | 3 | 0 | 0 |
| 22 | [Monotonic Stack](#monotonic-stack) | Find, for each element, the nearest smaller/larger element to its left or right using a monotonic stack instead of nested scans. | O(n) | O(n) | 3 | 0 | 0 | 3 | 0 |
